...
def snes_monitor(snes, iteration, norm):
    ...
    ksp_history.append(snes.getLinearSolveIterations())
...
plt.tight_layout()
plt.savefig("1d-steady-burgers-implicit.png", dpi=150)
plt.close()

print("\nSaved plot as 1d-steady-burgers-implicit.png")

# --------------------------------------------------
# KSP Iteration History Plot
# --------------------------------------------------

if len(ksp_history) > 0:
    plt.figure(figsize=(7, 4))
    iters = np.arange(len(ksp_history))
    plt.plot(iters, ksp_history, lw=2)
    plt.xlabel("SNES Iteration")
    plt.ylabel("Cumulative KSP Iterations")
    plt.title("KSP Iteration History")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("1d-steady-burgers-implicit-ksp.png", dpi=150)
    plt.close()

    print("Saved plot as 1d-steady-burgers-implicit-ksp.png")
...
if len(solution_history) > 1:
    ...
    ani.save("1d-steady-burgers-implicit.gif", dpi=150)
    plt.close()
    print("Animation saved as 1d-steady-burgers-implicit.gif")
